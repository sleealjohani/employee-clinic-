...
export async function runIntegration({ prisma, baseUrl, password }) {
  ...
  try {
    ...
    const login = await page("/login?mode=staff");
    ...
    async function signIn(username) {
      if (username.startsWith("test.employee")) {
        const i = Number(username.replace("test.employee", ""));
        const signed = await action("/login", "employeeLoginAction", "", {
          nationalId: syntheticId(i),
        });
        assert.equal(signed.response.status, 303);
        return signed.response.headers
          .getSetCookie()
          .map((s) => s.split(";")[0])
          .join("; ");
      }
    ...
    const { checkEmployeeLogin } = await import(
      "../tests/employee-login.integration.mjs"
    );
    ...
    const ohcSource = await ohcFixture({
      id: syntheticId(1),
      secondId: "9999999999",
      dose: "02/01/2026",
      received: "Yes",
    });
    const ohcFile = new File([ohcSource], "OHC-test.xlsx", {
      type: "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
    });
    const ohcPath = "/vaccinations/register";
    await page(ohcPath, admin);
    await action(ohcPath, "importOHCAction", staff, {
      file: ohcFile,
      mode: "preview",
    });
    assert.equal(
      await prisma.setting.count({ where: { key: "ohc.register" } }),
      0,
    );
    await action(ohcPath, "importOHCAction", admin, {
      file: ohcFile,
      mode: "preview",
    });
    assert.equal(
      await prisma.vaccination.count(),
      0,
      "preview creates no clinical records",
    );
    const ohcSha = createHash("sha256").update(ohcSource).digest("hex");
    const ohcVersion = createHash("sha256")
      .update(
        JSON.stringify({
          sha256: ohcSha,
          rows: [
    ...
    assert.equal(
      await prisma.vaccination.count(),
      1,
      "retry cannot duplicate source doses",
    );
    assert.equal((await fetch(baseUrl + "/api/ohc/export")).status, 401);
    for (const cookie of [viewer, employee])
      assert.equal(
        (
          await fetch(baseUrl + "/api/ohc/export", {
            headers: { Cookie: cookie },
          })
        ).status,
        403,
      );
    const sourceResponse = await fetch(baseUrl + "/api/ohc/export?original=1", {
    ...
    await Promise.all([
      action("/vaccinations", "createVaccinationAction", staff, {
        employeeId: "test_employee_1",
        vaccineCode: "HEP_B",
        doseNumber: "2",
        givenAt: "2026-02-02",
      }),
      action("/vaccinations", "createVaccinationAction", admin, {
        employeeId: "test_employee_2",
        vaccineCode: "OTHER",
        doseNumber: "1",
        givenAt: "2026-02-02",
    ...
    let ohcRegister = JSON.parse(
      (
        await prisma.setting.findUniqueOrThrow({
          where: { key: "ohc.register" },
        })
      ).value,
    );
    assert.equal(
      ohcRegister.doseCount,
      3,
      "concurrent dose writes both reach the saved workbook",
    );
    ...
    await savedBook.xlsx.load(saved);
    assert.equal(
      savedBook.getWorksheet("Data Base").getCell("Q3").value,
      "2: 02/02/2026",
    );
    ...
    for (const path of [
      "/employees/test_employee_1?tab=vaccines",
      "/vaccinations",
      "/dashboard",
      "/reports",
      "/due",
      ...
      assert.equal(
        (await page(path, path.startsWith("/portal") ? employee : admin))
          .response.status,
        200,
      );
    await action(ohcPath, "linkOHCRowAction", admin, {
      row: "4",
      employeeId: "test_employee_2",
      reason: "Verified synthetic identity",
      confirm: "yes",
    });
    ohcRegister = JSON.parse(
      (
        await prisma.setting.findUniqueOrThrow({
          where: { key: "ohc.register" },
        })
    ...
    assert.equal(
      ohcRegister.rows[1].nationalId,
      "9999999999",
      "manual resolution does not rewrite source identity",
    );
    const secondDose = await prisma.vaccination.findFirstOrThrow({
      where: { vaccineCode: "HEP_B", doseNumber: 2 },
    });
    await action("/employees/test_employee_1", "voidRecordAction", admin, {
      entity: "Vaccination",
      id: secondDose.id,
      reason: "Synthetic correction",
    });
    const exported = await fetch(baseUrl + "/api/ohc/export", {
      headers: { Cookie: staff },
    });
    assert.equal(exported.status, 200);
    assert.equal(exported.headers.get("cache-control"), "private, no-store");
    const voidBook = new ExcelJS.Workbook();
    await voidBook.xlsx.load(await exported.arrayBuffer());
    assert.equal(voidBook.getWorksheet("Data Base").getCell("Q3").value, null);
    assert.equal(
      voidBook.getWorksheet("Data Base").getCell("O3").value,
      "1: 02/01/2026",
    );
    assert.equal(
      (
        await prisma.vaccination.findUniqueOrThrow({
          where: { id: secondDose.id },
        })
      ).status,
      "ENTERED_IN_ERROR",
    );
    const sourceSetting = await prisma.setting.findUniqueOrThrow({
      where: { key: "ohc.source." + ohcSha },
    });
    await prisma.setting.update({
      where: { key: sourceSetting.key },
      data: { value: "broken" },
    });
    const beforeFailedSync = await prisma.vaccination.count();
    await action("/vaccinations", "createVaccinationAction", staff, {
      employeeId: "test_employee_1",
      vaccineCode: "MMR",
      doseNumber: "1",
      givenAt: "2026-03-01",
    ...
    assert.equal(
      await prisma.vaccination.count(),
      beforeFailedSync,
      "failed Excel sync rolls the dose back",
    );
    await prisma.setting.update({
      where: { key: sourceSetting.key },
      data: { value: sourceSetting.value },
    });
    assert.ok(
      await prisma.auditLog.count({
        where: { entity: "OHCRegister", action: "EXPORT" },
      }),
    );
    pass(
      "OHC source import, preview and stale guard; private exact-source export; concurrent dose sync, identity review, voiding and rollback",
    );
    ...
    pass(
      "staff/admin password and two-factor sign-in remain required; setup revokes previous sessions",
    );
