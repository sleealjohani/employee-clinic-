...
import { ageFrom, bmi, formatDate, formatDateTime, formatHijri } from "@/lib/format";
import { completeness, COMPLETENESS_LABELS, vitalOutOfRange, type CompletenessField } from "@/lib/clinical/rules";
...
import { Alert, Card, Chip, Empty, KeyValue, LinkButton, Meter, PageHeader, SectionTitle } from "@/components/ui";
...
import { ArchiveEmployeeButton, RestoreEmployeeButton } from "@/components/employee/ArchiveControls";
import { IconAllergy, IconPlus } from "@/components/layout/icons";

export const dynamic = "force-dynamic";

const TAB_KEYS: TabKey[] = ["overview", "visits", "labs", "allergies", "vaccines", "education", "notes"];
...
export default async function EmployeeRecordPage({
  ...
  const tab: TabKey = TAB_KEYS.includes(rawTab as TabKey) ? (rawTab as TabKey) : "overview";

  const employee = await db.employee.findUnique({
    where: { id },
    include: {
      visits: { where: { status: { not: "ENTERED_IN_ERROR" } }, orderBy: { visitDate: "desc" } },
      labResults: { orderBy: [{ collectedAt: "desc" }, { createdAt: "desc" }] },
      allergies: { orderBy: { recordedAt: "desc" } },
      vaccinations: { where: { status: "ACTIVE" }, orderBy: { givenAt: "desc" } },
      educations: { where: { status: "ACTIVE" }, orderBy: { providedAt: "desc" } },
      notes: { where: { status: "ACTIVE" }, orderBy: [{ isPinned: "desc" }, { createdAt: "desc" }] },
  ...
  const activeLabs = employee.labResults.filter((l) => l.status !== "ENTERED_IN_ERROR");
  ...
  const hepBDoses = employee.vaccinations.filter((v) => v.vaccineCode === "HEP_B").length;
  const hbv = hbvStatus(
    activeLabs.map((l) => ({
      testCode: l.testCode,
      flag: l.flag,
  ...
  if (activeLabs.some((l) => ["HIV_AGAB", "HBSAG", "ANTI_HCV", "ANTI_HBC_TOTAL"].includes(l.testCode))) {
    await writeAudit({
      user,
  ...
  return (
    <>
      <PageHeader
        title={employee.name}
        subtitle={[employee.jobTitle, employee.department].filter(Boolean).join(" · ") || undefined}
        badge={
          <>
            <Chip tone={employee.employmentStatus === "ACTIVE" ? "ok" : "neutral"} dot>
              {t(`empst.${employee.employmentStatus}`)}
            </Chip>
            {employee.isArchived && <Chip tone="neutral">{t("emp.archived")}</Chip>}
          </>
        }
        actions={
          <>
            <LinkButton href={`/employees?q=${encodeURIComponent(employee.nationalId)}`}>
              {t("action.back")}
            </LinkButton>
            {canEdit && <LinkButton href={`/employees/${employee.id}/edit`}>{t("action.edit")}</LinkButton>}
      ...
      {activeAllergies.length > 0 && (
        <div className="mb-4">
          <Alert tone={severeAllergy ? "danger" : "warn"} title={t("allergy.banner")}>
      ...
      {employee.isArchived && employee.archiveReason && (
        <div className="mb-4">
          <Alert tone="neutral">
            {t("action.archive")}: {employee.archiveReason} — {formatDate(employee.archivedAt, t.locale)}
          </Alert>
        </div>
      )}
      ...
      <div className="mb-4 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
        <Card>
          ...
          <div className="mt-2">
            <KeyValue label={t("emp.employeeNo")} value={employee.employeeNo ?? "—"} mono />
          </div>
        </Card>
        <Card>
          <KeyValue
            label={t("emp.bloodType")}
            value={employee.bloodType ? <span dir="ltr">{employee.bloodType}</span> : t("common.notRecorded")}
          />
          <div className="mt-2">
            <KeyValue
              label={t("emp.age")}
              value={ageFrom(employee.dob) ? `${ageFrom(employee.dob)}` : t("common.notRecorded")}
              mono
            />
          </div>
        </Card>
        <Card>
          <p className="text-[0.7rem] font-semibold" style={{ color: "var(--text-faint)" }}>
            {t("emp.hbvStatus")}
          </p>
          <div className="mt-1">
          ...
          {hbv.basis.length > 0 && (
            <p className="num mt-1.5 text-[0.68rem]" style={{ color: "var(--text-faint)" }} dir="ltr">
              {hbv.basis.join(" · ")}
            </p>
          )}
        </Card>
        <Card>
          ...
          {c.missing.length > 0 && (
            <p className="mt-1.5 text-[0.68rem]" style={{ color: "var(--text-faint)" }}>
              {t("emp.missingFields")}:{" "}
              {c.missing.map((f: CompletenessField) => COMPLETENESS_LABELS[f][t.locale]).join("، ")}
            </p>
          )}
        </Card>
      </div>

      <RecordTabs employeeId={employee.id} active={tab} labels={labels} counts={counts} />
        ...
        <div className="grid gap-4 lg:grid-cols-3">
          <Card className="lg:col-span-2">
            ...
            <dl className="grid gap-3 sm:grid-cols-3">
              <KeyValue label={t("emp.nameEn")} value={employee.nameEn ?? "—"} />
              <KeyValue label={t("emp.gender")} value={employee.gender ? t(`gender.${employee.gender}`) : "—"} />
              <KeyValue
                label={t("emp.dob")}
                value={
                  employee.dob ? (
              ...
              />
              <KeyValue label={t("emp.phone")} value={<span dir="ltr">{employee.phone ?? "—"}</span>} mono />
              <KeyValue label={t("emp.email")} value={<span dir="ltr">{employee.email ?? "—"}</span>} />
              <KeyValue label={t("emp.hireDate")} value={formatDate(employee.hireDate, t.locale)} mono />
              <KeyValue
                label={t("emp.chronic")}
                value={employee.chronicConditions.length ? employee.chronicConditions.join("، ") : t("common.none")}
              />
              <KeyValue
                label={t("emp.medications")}
                value={
                  employee.currentMedications.length ? employee.currentMedications.join("، ") : t("common.none")
                }
              />
              <KeyValue
                label={t("emp.lastVisit")}
                value={lastVisit ? formatDate(lastVisit.visitDate, t.locale) : t("common.none")}
                mono
              />
              <KeyValue
                label={t("emp.lastLab")}
                value={lastLab ? formatDate(lastLab.collectedAt, t.locale) : t("common.none")}
                mono
              />
            </dl>
          </Card>
          ...
          <Card>
            ...
            <ul className="space-y-2.5">
              {OCCUPATIONAL_VACCINES.map((vac) => {
                const doses = employee.vaccinations.filter((v) => v.vaccineCode === vac.code);
                const next = nextVaccineDue(vac.code, doses);
                const overdue = next && next.dueDate < new Date();
                return (
                  <li key={vac.code} className="flex items-center justify-between gap-2 text-sm">
                    <span className="min-w-0 truncate">{t.locale === "ar" ? vac.nameAr : vac.nameEn}</span>
                    {doses.length === 0 ? (
                  ...
                  <tbody>
                    {employee.employmentHistory.map((h) => (
                      <tr key={h.id}>
                        ...
                        <td className="num">{formatDate(h.effectiveFrom, t.locale)}</td>
                        <td className="num">{h.effectiveTo ? formatDate(h.effectiveTo, t.locale) : "—"}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
                ...
                return (
                  <details key={visit.id} className="border-b last:border-b-0">
                    <summary className="flex cursor-pointer flex-wrap items-center gap-3 px-4 py-3 hover:bg-[var(--surface-2)]">
                      <span className="num text-sm font-semibold">{formatDate(visit.visitDate, t.locale)}</span>
                    ...
                    <div className="border-t px-4 py-4" style={{ background: "var(--surface-2)" }}>
                      <dl className="grid gap-3 sm:grid-cols-4">
                        <KeyValue label={t("visit.temp")} value={visit.tempC ?? "—"} mono />
                        <KeyValue
                          label={t("visit.bp")}
                          value={visit.systolic && visit.diastolic ? `${visit.systolic}/${visit.diastolic}` : "—"}
                          mono
                        />
                        <KeyValue label={t("visit.pulse")} value={visit.pulse ?? "—"} mono />
                        <KeyValue label={t("visit.spo2")} value={visit.spo2 ?? "—"} mono />
                        <KeyValue label={t("visit.rr")} value={visit.respRate ?? "—"} mono />
                        <KeyValue label={t("visit.weight")} value={visit.weightKg ?? "—"} mono />
                        <KeyValue label={t("visit.height")} value={visit.heightCm ?? "—"} mono />
                        <KeyValue label={t("visit.bmi")} value={b ?? "—"} mono />
                      </dl>
                      <dl className="mt-4 grid gap-3 sm:grid-cols-2">
                        <KeyValue label={t("visit.chief")} value={visit.chiefComplaint ?? "—"} />
                        <KeyValue label={t("visit.diagnosis")} value={visit.diagnosis ?? "—"} />
                        <KeyValue label={t("visit.plan")} value={visit.plan ?? "—"} />
                        <KeyValue label={t("common.notes")} value={visit.notes ?? "—"} />
                      </dl>
                      {canVoid && (
          ...
          {employee.labResults.length === 0 ? (
            ...
            <div className="border-t">
              {employee.labResults.map((lab) => (
                <LabResultRow key={lab.id} lab={lab} t={t} canWrite={canWrite} canVoid={canVoid} />
              ))}
            </div>
          )}
        </Card>
                ...
                <tbody>
                  {employee.allergies.map((a) => (
                    <tr key={a.id} style={{ opacity: a.allergyStatus === "ACTIVE" ? 1 : 0.55 }}>
                      ...
                      <td className="num">{formatDate(a.recordedAt, t.locale)}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
          ...
          <Card>
            ...
            <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
              {OCCUPATIONAL_VACCINES.map((vac) => {
                const doses = employee.vaccinations.filter((v) => v.vaccineCode === vac.code);
                const next = nextVaccineDue(vac.code, doses);
                const pct = Math.min(100, Math.round((doses.length / vac.doses) * 100));
                const overdue = next && next.dueDate < new Date();
                return (
                  <div key={vac.code} className="rounded-xl border p-3">
                    <p className="text-sm font-bold">{t.locale === "ar" ? vac.nameAr : vac.nameEn}</p>
                    <p className="num mt-0.5 text-xs" style={{ color: "var(--text-faint)" }}>
                    ...
                    <div className="mt-2">
                      {next ? (
                        <Chip tone={overdue ? "danger" : "warn"}>
                          {t("vac.nextDue")}: {formatDate(next.dueDate, t.locale)}
                        </Chip>
                      ) : (
                        <Chip tone="ok" dot>
                ...
                <tbody>
                  {employee.educations.map((e) => (
                    <tr key={e.id}>
                      ...
                      <td>{e.method ?? "—"}</td>
                      <td className="num">{formatDate(e.providedAt, t.locale)}</td>
                      <td>{e.notes ?? "—"}</td>
                    </tr>
                  ))}
                </tbody>
              ...
              {employee.notes.map((n) => (
                <li key={n.id} className="border-b px-4 py-3 last:border-b-0">
                  <div className="mb-1 flex items-center gap-2">
                    {n.isPinned && <Chip tone="accent">{t("note.pinned")}</Chip>}
      ...
      <p className="mt-6 text-[0.68rem]" style={{ color: "var(--text-faint)" }}>
        ...
        <span className="num">{formatDateTime(employee.createdAt, t.locale)}</span>
        {" · "}
        <Link href={`/audit?entity=Employee&entityId=${employee.id}`} style={{ color: "var(--accent-text)" }}>
          {t("audit.title")}
        </Link>
      </p>
    </>
