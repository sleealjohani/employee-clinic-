...
import { releaseLabAction } from "@/server/actions/clinical";
...
function labPresentation(lab: LabResult, t: Translator) {
  ...
  const { interpretation, action } = interpretLab(
    lab.testCode,
    lab.flag,
    lab.valueNum,
    t.locale,
  ...
  const value =
    lab.resultType === "QUANTITATIVE"
      ? `${{ EQ: "", LT: "<", LE: "≤", GT: ">", GE: "≥" }[lab.comparator]}${formatValue(lab.valueNum)}${lab.unit ? ` ${lab.unit}` : ""}`
      : (lab.valueText ?? "—");
...
export function LabResultDetails({
  ...
  const { name, tone, interpretation, action, critical, voided, value, range } =
    labPresentation(lab, t);
  ...
  return (
    <article style={{ opacity: voided ? 0.55 : 1 }}>
      <header className="mb-4 flex flex-wrap items-start justify-between gap-3 border-b pb-4">
        <div className="min-w-0">
          <div className="mb-2 flex flex-wrap items-center gap-2">
            ...
            {critical && !lab.criticalNotifiedAt && (
              <Chip tone="danger">{t("lab.critical")}</Chip>
            )}
            {lab.requiresReview && !lab.reviewedAt && (
              <Chip tone="warn">{t("lab.needsReview")}</Chip>
            )}
            {voided && (
              <Chip tone="neutral">{t("recordStatus.ENTERED_IN_ERROR")}</Chip>
            )}
          </div>
          <h3 className="text-lg font-extrabold">{name}</h3>
          <p className="num mt-1 text-xl font-black" dir="ltr">
            {value}
          </p>
        ...
        <div
          className="rounded-xl border px-3 py-2 text-end"
        ...
        >
          ...
          <strong className="num mt-1 block text-sm" dir="ltr">
            {range}
          </strong>
        </div>
      </header>
      ...
      <section className="mt-4">
        <p
          className="mb-2 text-[0.67rem] font-extrabold uppercase tracking-[0.06em]"
          style={{ color: "var(--text-faint)" }}
        >
        ...
        <dl className="grid gap-2 sm:grid-cols-2 xl:grid-cols-3">
          <div className="rounded-xl border p-3">
            <KeyValue
              label={t("lab.collectedAt")}
              value={formatDate(lab.collectedAt, t.locale)}
              mono
            />
          </div>
          <div className="rounded-xl border p-3">
            <KeyValue
              label={t("lab.verifiedAt")}
              value={formatDate(lab.verifiedAt, t.locale)}
              mono
            />
          </div>
          <div className="rounded-xl border p-3">
          ...
          <div className="rounded-xl border p-3">
            <KeyValue
              label={t("lab.performedBy")}
              value={lab.performedBy ?? "—"}
            />
          </div>
          <div className="rounded-xl border p-3">
            <KeyValue
              label={t("lab.verifiedBy")}
              value={lab.verifiedBy ?? "—"}
            />
          </div>
          <div className="rounded-xl border p-3">
            ...
            <KeyValue
              ...
              value={
                lab.sourceAttachmentId ? (
                  <a
                    href={`/documents/${lab.sourceAttachmentId}${lab.sourcePage ? `?page=${lab.sourcePage}` : ""}`}
                    target="_blank"
                    rel="noreferrer"
      ...
      {lab.criticalNotifiedAt && (
        <div
          ...
          style={{
            ...
            color: "var(--ok)",
            borderColor: "color-mix(in srgb, var(--ok) 18%, var(--border))",
          }}
        >
      ...
      {canWrite && !voided && (
        <div className="mt-5 flex flex-wrap gap-2 border-t pt-4 no-print">
          ...
          {!lab.reviewedAt && (
            <ReviewLabButton labId={lab.id} label={t("lab.markReviewed")} />
          )}
          {lab.reviewedAt && (!critical || lab.criticalNotifiedAt) && (
            <ActionForm
              action={releaseLabAction}
              label={t(
                lab.releasedAt ? "v2.withdrawResult" : "v2.releaseResult",
              )}
            >
              ...
              <input
                type="hidden"
                name="release"
                value={lab.releasedAt ? "no" : "yes"}
              />
            </ActionForm>
          )}
          {lab.releasedAt && <Chip tone="ok">{t("v2.visibleToEmployee")}</Chip>}
          {canVoid && (
            <Modal
              title={t("action.void")}
              trigger={
