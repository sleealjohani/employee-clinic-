...
export const LOCAL_EXTRACTION_MODEL = "local-pdf-rules-v2";
export const LOCAL_PROMPT_VERSION = "local-pdf-rules-2026-09-1";
...
function findTest(line: string) {
  ...
  for (const matcher of MATCHERS) {
    ...
    if (
      ...
      (candidate.start === best.start && candidate.weight > best.weight)
    )
      best = candidate;
  }
  return best;
...
async function readPages(bytes: Buffer): Promise<PageText[]> {
  ...
  if (pdf.numPages > 500) {
    await task.destroy();
    throw new Error("imp.tooManyPages");
  }
...
function identity(lines: string[], text: string) {
  ...
  if (!id) {
    const candidates = [...new Set(text.match(/\b[12]\d{9}\b/g) ?? [])].filter(
      (value) => validateNationalId(value).valid,
    );
  ...
  return {
    ...
    name,
    employeeNo,
    confidence: id ? confidence : name || employeeNo ? 0.78 : 0.64,
  };
}
...
function dateFrom(value: string) {
  ...
  if (hit) {
    ...
    if (a > 12 && b <= 12)
      return `${hit[3]}-${String(b).padStart(2, "0")}-${String(a).padStart(2, "0")}`;
    if (b > 12 && a <= 12)
      return `${hit[3]}-${String(a).padStart(2, "0")}-${String(b).padStart(2, "0")}`;
  }
  return "";
}
...
function meta(lines: string[]) {
  return {
    ...
    orderNo: labelled(lines, [
      /(?:order\s*(?:no\.?|number)|request\s*(?:no\.?|number)|رقم\s*(?:الطلب|الأمر))\s*[:：#\-]?\s*([A-Z0-9\-/]+)/iu,
    ]),
    sampleNo: labelled(lines, [
      /(?:sample\s*(?:no\.?|number)|specimen\s*(?:no\.?|number)|accession\s*(?:no\.?|number)|رقم\s*(?:العينة|العينه))\s*[:：#\-]?\s*([A-Z0-9\-/]+)/iu,
    ]),
    performedBy: cutAtNextLabel(
      labelled(lines, [
        /(?:performed\s*by|technician|منفذ\s*التحليل)\s*[:：#\-]?\s*(.+)$/iu,
      ]),
    ),
    verifiedBy: cutAtNextLabel(
      labelled(lines, [
        /(?:verified\s*by|validated\s*by|approved\s*by|معتمد\s*النتيجة)\s*[:：#\-]?\s*(.+)$/iu,
      ]),
    ),
    labName: cutAtNextLabel(
      labelled(lines, [
        /(?:laboratory|lab\s*name|المختبر)\s*[:：#\-]?\s*(.+)$/iu,
      ]),
    ),
  };
}
...
function qualitative(text: string) {
  ...
  for (const term of [...QUAL].sort((a, b) => b.length - a.length)) {
    const hit = new RegExp(
      `(^|[^\\p{L}\\p{N}])(${esc(term)})(?=$|[^\\p{L}\\p{N}])`,
      "iu",
    ).exec(lower);
    if (hit) {
      const at = hit.index + (hit[1]?.length || 0);
      return text.slice(at, at + term.length).trim();
    }
  }
  return "";
}
...
function normaliseUnit(value: string) {
  return value.toLocaleLowerCase().replace(/[µμ]/g, "u").replace(/\s/g, "");
}

function refRange(text: string) {
  const hit =
    /([+-]?(?:\d+(?:[.,]\d+)?|[.,]\d+))\s*(?:-|–|—|to)\s*([+-]?(?:\d+(?:[.,]\d+)?|[.,]\d+))/iu.exec(
      text,
    );
  return hit
    ? {
...
function parse(
  line: string,
  page: number,
  metadata: ReturnType<typeof meta>,
  ...
  const unit =
    printedUnit?.replace(/\s/g, "") ||
    (test.def.unit && normaliseUnit(line).includes(normaliseUnit(test.def.unit))
      ? test.def.unit
      : "");
  return {
    ...base,
...
export async function extractLocalPdfReport(
  bytes: Buffer,
  ...
  const textPages = pages.filter(
    (page) => page.text.replace(/\s/g, "").length >= 24,
  ).length;
  ...
  const covered = new Set(
    reports.flatMap((r) => r.results.map((item) => item.page)),
  );
  return {
    reports,
    ...
    unreadPages: pages.filter((p) => !covered.has(p.page)).map((p) => p.page),
  };
}
